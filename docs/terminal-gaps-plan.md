...
(`parent-stage-mismatch` 사전 차단) / **T-J** 조그레스 전용(분할 편입 판단 필요).
...
`evolvesFrom: [{from, when}]` 는 부모가 하나뿐이라 "둘 다 필요"를 표현할 수 없다.
그래서 **판단이 필요한 항목**으로 따로 묶고 `--check` 게이트에서 뺀다.
**편입 불가라는 뜻은 아니다** — 부모별로 쪼개 독립 엣지로 넣는 선례가 있다.

**실측 선례 (2026-09-02)**

| 사례 | 내용 |
|---|---|
| 오메가몬 | 워그레이몬+메탈가루몬 조그레스인데, Wikimon `l1=Ultimate` 를 **초궁극체로 승격**시켜 두 부모와 스테이지를 인접하게 만들고 각각을 독립 엣지로 넣었다. 같은 승격이 **15종**에 적용돼 있다 (§7 "초궁극체 칸 승격") |
| 조그레스 전용 엣지 | 이미 **2건** 들어와 있다 (`weregarurumon→boltmon`, `renamon→hanumon`) |
| 반대 방향 근거 | 그래프 엣지 960건 중 **958건(99.8%)** 은 단독 가능한 줄을 갖는다 — 조그레스 편입은 예외적 판단이고 기본값이 아니다 |

초판은 이 구분이 없어 **그레이스노바몬을 T-B 양방향 확정(즉시 조치 가능)으로
