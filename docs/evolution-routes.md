...
- `topStage`는 **이 라인이 실제로 어디까지 가는가**를 적는다. 걷기를 그 단계에서 멈추게 해
  없는 스테이지를 넘겨보지 않게 한다. 로테이션 자격 판정에는 쓰이지 않으므로(D12) 선언해도
  손해가 없다.
...
| R2 | 후보 조건 = **최소 궁극체까지 엣지가 이어짐** AND **경로상 모든 노드의 `sprite` 도트가 존재** | ✅ `pruneTree` |
...
| R6 | 루트는 항상 **궁극체 이상**에서 끝난다 (R2의 귀결). 초궁극체 도달은 요구하지 않는다 | ✅ 가짜 엣지 없이 달성 |
...
- **backward** — 노드가 살아남는 조건 = 자기 도트 존재 AND (**궁극체 이상**이거나, 살아남은
  후속이 하나 이상). 완전기 이하에서 끊기는 분기가 여기서 빠진다.
...
**주의 — zip 안에 열화본이 섞여 있다.** 스레드가 Photobucket에서 이미지를 회수해 재업로드한
것이라, 일부 시트는 워터마크가 박히고 초점이 흐려진 상태다(`387_MirageGaogamonBurst`가
그렇다 — 도트를 뽑을 수 없다). 또 `415_ChaosGallantmonCore`처럼 **필드 스프라이트 없이 대형
전투 포즈만** 있는 시트도 있다. 트리에 넣기 전에 `--rows`로 필드 행의 존재와 선명도를 먼저
확인한다.
...
**시트 파일명은 한글명으로 검색하면 안 된다.** 세 번 놓쳤다: `샤인그레이몬`은 파일명이
`374_ShineGreymonGarmmon`(`Garmmon`은 디지몬이 아니라 시트 제작자 이름), `프레리몬`은
`290_Prariemon`(`i` 빠진 오타), `크리피몬`은 `403_Daemon`(크리피몬은 영어 더빙명). 없다고
결론 내리기 전에 **목록 전체를 훑는다** — grep은 이름이 정확할 때만 유효하다.
...
| D2 | ~~천장 미달 루트는 추첨하지 않는다(B안)~~ → **D12로 완화** | 2M 도달일에 천장을 못 보는 일이 없어야 한다는 취지였으나, 대가가 너무 컸다(D12) |
...
| D14 | 팔코몬에 레이브몬 분기 추가 (`야타가라몬 → 레이브몬 → 레이브몬 버스트 모드`) | 캐논상 팔코몬의 본계보인데 팩은 크로노몬 홀리 모드 쪽만 갖고 있었다. 팩이 이미 있어 형제 분기 2개로 1→2루트가 됐다. 발두르몬에 `focus`, 레이브몬에 `dark`를 줘 다른 팩과 같은 분기 성격을 맞췄다. 버스트 모드의 정면 행은 IoU도 눈 색도 판별에 실패해(차이 +0.02, 눈이 32px에 렌더되지 않음) **최종 32px 도트를 확정된 레이브몬 도트와 대조**해 골랐다 — 살색 얼굴 패치가 판별자였다 |
| D13 | 가오몬 팩 추가 (와냐몬 → 가오몬 → 가오가몬 → 마하가오가몬 → 미라지가오몬) | D12가 실제로 무엇을 풀었는지 보여주는 첫 사례다. 미라지가오몬 버스트 모드가 캐논에 있지만 시트가 Photobucket 열화본이라 못 쓰는데, 예전 규칙이면 이 팩은 통째로 벤치였다. 지금은 `topStage: "ultimate"`로 궁극체에서 끝나고 로테이션에 들어간다 — 2M을 넘겨도 한 칸 더 안 갈 뿐이다. 로테이션 9→10팩 |
| D12 | 천장 도달 요구를 **최소 궁극체 도달**로 완화 | D2의 요구는 로테이션 자격을 원작 로스터의 성질에 묶어버렸다 — 합체·모드체인지 형태가 420종 중 19종뿐이라, 도트를 얼마나 뽑든 성장기 60종의 대부분은 영원히 추첨에 못 들어왔다. 얻는 것에 비해 잃는 게 컸다: 궁극체에서 끝나는 라인은 2M을 넘겨도 한 칸 더 안 갈 뿐이다. **완전기는 여전히 제외** — 하루 종일 두 단계 뒤처져 보이는 건 짧은 라인이 아니라 고장으로 읽힌다. 즉시 효과는 길몬 다크 루트 개통(14→15) |
...
| 길몬 | 카오스듀크몬 + 카오스듀크몬 코어 | ✅ 보유 (`358_ChaosDukemon`, `415_ChaosGallantmonCore`) | **+1** — 블랙메가로그라우몬의 막다른 분기를 천장까지 잇는다 |
| 브이몬 | 황제드라몬 드래곤 모드 블랙 | ✅ 보유 (`334_ImperialdramonDragonBlack`) | dark 분기 후보 |
| 아구몬 | 샤인그레이몬 루인 모드 | ✅ 보유 (`386_ShineGreymonRuin`) | dark 분기 후보 |
| 임프몬 | 크리피몬(=Daemon) | ✅ 보유 (`305_Demon`, `403_Daemon`) | 배치 미정 |
| 임프몬 | 소서리몬 | ✅ 보유 (`130_Sorcerymon`) | 배치 미정 |
| 임프몬 | 구울몬, 미미몬 | ❌ withthewill 541개에 없음 | — |
| 임프몬 | 로치몬, 바이럴몬, 메가쿠와가몬 블루 | ⚠️ 한글 표기로 목록의 어느 항목인지 특정 불가 | 표기를 원어명으로 다시 확인해야 한다 |
| 테리어몬 | 프레리몬 | ✅ 보유 (`290_Prariemon` — 파일명이 `Prarie`로 오타) | +0 — 아머체라 후속이 없어 R2가 제외한다 |
...
| 길몬 | ✅ | superultimate | 1 |
| 케라몬 | ✅ | superultimate | 1 |
| 팔코몬 | ✅ | superultimate | 2 (크로노몬 홀리 모드 / 레이브몬 버스트 모드) |
...
| 성장기 = 팩 후보 | **60종** (현재 팩 10개) |
| 로컬 시트 보유 | 55 (PICKS 사용 52, 미사용 3: `saberleomon`·`chaosdukemon_core`·`puttimon`) |
| 트리 노드 | 73 (분기로 늘어난 노드 18) |
| 하루 루트 | 16 |

**D12 이전의 병목은 최상위(모드체인지·합체) 형태였다.** 이 프로젝트의 `superultimate`는
게임에 없는 7번째 단계라 합체체·버스트/모드체인지만 자격이 있고, 로스터 420종 중 그런
형태는 **19종**뿐이다:
...
즉 D12 이전에는 성장기가 60종이어도 **R2를 통과할 팩이 10여 개 상한**이었다. 로테이션
자격이 원작 로스터의 성질에 묶여 있었던 것이다.

**D12로 이 상한이 풀렸다.** 지금 자격 조건은 "궁극체까지 도트가 있는가"이므로, 병목이
`19종의 최상위 형태` → `궁극기+ 117 / 완전기 123종`으로 옮겨졌다. 성장기 60종의 대부분이
실제 후보가 된다. 남은 비용은 도트 추출 노동뿐이고, 그건 §6 절차로 노드당 1~2분이다.

### 아직 안 만든 팩 중 사슬이 성립하는 것

가오몬(D13)·팔코몬 레이브몬 분기(D14)는 **추가 완료**. 남은 것은 1개다:
...
최상위 스테이지는 `superultimate`, 루트가 끝날 수 있는 최소 단계는 `ultimate`. 로테이션 10팩 / 유효 10팩.
...
**가오몬** (`gaomon`) — 천장 `ultimate` · 트리 없음(`stageNames` 고정선)

```
가오몬 → 가오가몬 → 마하가오가몬 → 미라지가오몬
```
