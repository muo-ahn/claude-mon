...
- **Q5.** 임프몬의 7개 백로그 노드(§10)가 전부 `베르제브몬 블래스트 모드` 도트 하나에
  막혀 있다. 단일 백로그 항목 중 가장 크다. DWDS에 해당 시트가 있는지 확인이 먼저다.
  없으면 (a) 다른 출처, (b) 임프몬도 `topStage: "ultimate"` 선언 — 단 (b)는 캐논상 위 단계가
  존재하는 라인에 천장을 선언하는 것이므로 D7의 구분을 흐린다.
...
| 팩 | 추가 노드 | 시트 | 루트 증가 |
|---|---|---|---|
| 테리어몬 | 블랙라피드몬, 프레리몬, 세이버레오몬, 블랙메가가르고몬 | ✅ 48570/48692/48454/48706 | **+2 (즉시)** |
| 레나몬 | 쿠즈하몬 | ✅ 48778 | **+1 (즉시)** |
| 파피몬 | 블랙워가루몬 (D4) | ✅ 48678 | +0 (기존 다크드라몬 루트를 정상화) |
| 길몬 | 카오스듀크몬 | ✅ 48621 | +0 (듀크몬 크림슨 모드로 이어지지 않아 R2가 제외) |
| 임프몬 | 로치몬, 바이럴몬, 소서리몬, 메가쿠와가몬 블루, 미미몬, 크리피몬, 구울몬 | ✅ 전부 | Q5 해결 시 +3 |
