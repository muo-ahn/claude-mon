...
PICKS = {
    ...
    # 가오몬 라인은 전 단계를 DWDS에서 뽑는다. 시트가 전부 격자 배치이고
    # 정면 행이 시트마다 다르다 — 와냐몬은 노란 눈이 보이는 r3, 가오몬은
    # 얼굴과 빨간 머리띠가 보이는 r0, 가오가몬은 흰 주둥이가 보이는 r0,
    # 마하가오가몬은 등의 부스터가 정면 공격 포즈(r2)와 같은 모양으로 보이는
    # r0, 미라지가오몬은 투구가 보이고 붉은 망토가 뒤로 가는 r2다.
    #
    # 초궁극체(미라지가오몬 버스트 모드)는 캐논에 있지만 시트가 Photobucket
    # 열화본이라 쓸 수 없다 -- pack.json이 topStage를 ultimate로 선언한다.
    "gaomon": {
        "idle": ("gaomon", [0, 1], 0),
        "baby": ("wanyamon", [0, 1], 3),
        "child": ("gaomon", [0, 1], 0),
        "adult": ("gaogamon", [0, 1], 0),
        "perfect": ("machgaogamon", [0, 1], 0),
        "ultimate": ("miragegaogamon", [0, 1], 2),
    },
    "keramon": {
        "idle": ("keramon", [3, 4]),
        "baby": ("kuramon", [3, 4]),
