...
// Single source of truth for what checkCondition's switch actually handles.
// validatePackTree (lib/daily.js) checks pack.json condition types against
// this list instead of duplicating the switch's case names, so the two
// can't drift apart.
const KNOWN_CONDITION_TYPES = [
  'always',
  'toolSuccessCount',
  'globalToolSuccessCount',
  'errorRatePct',
...
function checkCondition(cond, ctx) {
  ...
  switch (cond.type) {
    ...
    case 'dailyUserTurns':
      return (ctx.dailyUserTurns || 0) >= cond.gte;
    case 'outputPerTurn': {
      const val = ctx.outputPerTurn || 0;
...
// `all: [...]` requires every sub-condition; `any: [...]` (its symmetric
// counterpart - the "설계 날" gate needs an OR between the usual token
// threshold and the sage-day alternative) requires just one. A bare
// condition is an implicit single-element `all`, and `and` is a two-item
// alias kept for pack.json files written before `all` existed.
// `undefined`/`null` (no condition declared - the guaranteed edge) and
// `all: []` both mean "always met"; `any: []` means "never met".
function conditionMet(condition, ctx) {
  ...
  if (Array.isArray(condition.any)) {
    return condition.any.some((c) => conditionMet(c, ctx));
  }
