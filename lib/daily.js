...
// The daily rotation pool (D7 - see docs/evolution-routes.md §8 D7): a
// valid pack is a rotation candidate if either
//
//   (a) readTree resolves a tree for it at all - front-contiguous is
//       enough (see readTree's doc comment); a line stopping short of the
//       global top stage (가오몬's 궁극체 ceiling) is included now, not
//       held out. `terminal: true` on that line's last node is what makes
//       it a real ending rather than a data gap - validatePackTree's
//       missing-terminal rule is the contract for that, checked by tests
//       and scripts/build-evolution-map.js, never at runtime (decision E).
//   (b) it has no tree at all (legacy stageNames-only pack) but still
//       declares the global top stage by name.
//
// Plain listValidPacks would be too permissive here: it only checks
// idle-0.png and a Digi-Egg sprite, so a directory with a missing or
// malformed pack.json would still be a "valid pack" and would ride into
// the rotation on nothing at all. Requiring (a) or (b) keeps that out
// while admitting every real short line.
function listRotationPacks(packsDir, sharedDir) {
  const top = topStageId();
  return listValidPacks(packsDir, sharedDir).filter((pack) => {
    if (readTree(packsDir, pack)) return true;
...
// ever sees the normalized shape. The spread preserves any other field the
// pack.json node carries (`terminal` included) untouched.
...
// A line's actual ending, declared by the pack rather than inferred from
// its shape. `evolutions: []` looks the same on a node whose author meant
// "this is where the line ends" and on a legacy node that simply forgot to
// declare `next`/`evolutions` at all - normalizeNode turns both into the
// same empty array, so the empty array can't be trusted to mean "done" (a
// forgotten edge would silently read as a valid terminal line instead of
// the data bug it is). `terminal: true` is unambiguous either way it's
// checked - by walk (never fall through to candidatesFor's spine-return
// fallback past this node) or by validatePackTree (this node is exempt
// from needing its own guaranteed edge, and any edges it does declare
// anyway are a contradiction - see the terminal-with-edges rule).
//
// Strict `=== true` on purpose: a hand-edited pack.json stringifying it as
// `"true"` should fail closed into "not terminal" rather than silently
// working, the same way CACHE_VERSION's exact-match check does above.
...
//
// D7 relaxation: a tree no longer has to declare all 7 global stages. What
// it does have to be is *front-contiguous* - every stage from digitama up
// to wherever it stops, with no hole in the middle. A line that legitimately
// ends early (가오몬 tops out at 궁극체 - no 초궁극체 sprite exists to draw)
// just stops declaring stages past that point; walk()/selectRoute() only
// ever see the stages this function actually returns, and the line's last
// node is expected to carry `terminal: true` (validatePackTree's
// missing-terminal rule is the enforcement for that, not this function -
// this function only cares about shape). A genuine hole - some stage
// declared *after* a break in the ladder - is rejected outright, because
// candidatesFor/byId span every returned stage positionally and a hole
// would resolve silently into the wrong stage's slot (the same
// stage-skip hazard validatePackTree's stage-skip rule exists to catch,
// but for a hole this function is the one place positioned to catch it -
// once a tree comes back from here, every stage in it is assumed
// contiguous everywhere else that reads it).
function readTree(packsDir, pack) {
  try {
    ...
    const declared = (id) => Array.isArray(tree[id]) && tree[id].length > 0;
    const declaredIds = [];
    for (const stage of TREE.stages) {
      if (!declared(stage.id)) break; // front-contiguous prefix ends here
      declaredIds.push(stage.id);
    }
    if (declaredIds.length === 0) return null; // not even digitama declared

    // A stage past the break that's still declared is a hole, not a short
    // line - reject the whole tree rather than silently drop it.
    for (let i = declaredIds.length; i < TREE.stages.length; i++) {
      if (declared(TREE.stages[i].id)) return null;
    }

    const normalized = {};
    for (const stageId of declaredIds) {
...
//
//   missing-unconditional-edge - a non-terminal, non-global-top node whose
//     last (or only) edge isn't `when: null`/undefined, so nothing
//     guarantees it ever reaches a real ending (decision C). Exempt: a
//     node with `terminal: true` (it IS the ending, by declaration - see
//     isTerminal), and a node at the tree's global top stage regardless of
//     `terminal` (D7: this exemption is unconditional only there, unlike
//     every other stage - see docs/evolution-routes.md §8 D7 for why the
//     old `topStage = stages[stages.length - 1]` (the tree's OWN last
//     declared stage) can no longer be what grants this: a line stopping
//     short of the global top, like 가오몬's, would otherwise skip this
//     rule at its own ceiling without ever having to say so via
//     `terminal`, and the flag would stop being a real contract).
//   terminal-with-edges        - `terminal: true` but the node still lists
//     edges - contradiction: it can't be both "this is where the line
//     ends" and "here's where it goes next".
//   missing-terminal           - a node at this tree's own last declared
//     stage, which isn't the global top stage, that isn't marked
//     terminal. The line stops there either way (there is nothing after
//     it in this tree), so silence about why is the actual bug -
//     `terminal: true` is what turns "a line stops short of one" into a
//     shape validatePackTree can call correct.
...
//
// Deliberately NOT a rule here: a hole in the middle of a tree's declared
// stages ("stage-gap"). That is readTree's load-time concern (front-
// contiguous or reject, see its doc comment) - this function only ever
// sees trees callers hand it directly (tests, scripts/build-evolution-
// map.js), which may legitimately have gaps of their own for reasons
// unrelated to D7 (see MINIMAL_TREE in test/daily.test.js). Mixing that
// layer in here would make this function reject fixtures its own contract
// never claimed to police.
function validatePackTree(tree) {
  const violations = [];
  const stages = TREE.stages.map((s) => s.id).filter((id) => Array.isArray(tree[id]) && tree[id].length > 0);
  const declaredTop = stages[stages.length - 1];
  const globalTop = topStageId();
  const byId = new Map();
  ...
  for (let i = 0; i < stages.length; i++) {
    ...
    for (const node of tree[stage]) {
      ...
      if (terminal && edges.length > 0) {
        violations.push({ rule: 'terminal-with-edges', stage, node: node.id });
      }

      if (stage === declaredTop && stage !== globalTop && !terminal) {
        violations.push({ rule: 'missing-terminal', stage, node: node.id });
      }

      if (stage === globalTop || terminal) continue; // rule 5 / terminal exemption
      ...
      for (const edge of edges) {
        ...
        } else if (nextStage && !tree[nextStage].some((n) => n.id === edge.to)) {
...
function walk(dateKST, pack, tree, ctx, byId, locked, lockedThroughIdx) {
  const route = {};
  let current = null;
  ...
  for (const stageDef of TREE.stages) {
    ...
    // The line already ended (see isTerminal) - every stage from here on
    // repeats that same node rather than asking candidatesFor for a
    // fallback. This has to run before the locked branch below too: a
    // lazily-redrawn route can carry the locked prefix straight through
    // the terminal node and into stages beyond it, and by the time this
    // loop gets there `current` is already the terminal node regardless
    // of which branch set it. Skipping this would revive exactly the
    // spine-return bug D1 removed - candidatesFor's fallback on a
    // terminal node (zero edges) is `tree[stage][0]`, the NEXT stage's
    // spine, so the mon would flip to an unrelated line's form the
    // instant it reached its own ending. For a pack whose tree doesn't
    // even declare `stage` (가오몬 past 궁극체), that same fallback would
    // throw on `undefined[0]` instead - this branch is what keeps that a
    // repeat instead of a crash.
    if (current && isTerminal(current)) {
      route[stage] = { id: current.id, name: current.name, sprite: current.sprite };
      continue;
    }
    ...
    const declared = Array.isArray(tree[stage]) && tree[stage].length > 0;
    if (!declared) {
      // This pack's tree stops here without `current` being terminal - a
      // data violation (validatePackTree's missing-terminal rule catches
      // it) but the runtime never throws over bad data (decision E).
      // Degrade to repeating the last node reached rather than indexing
      // into a stage this tree never declared.
      if (current) route[stage] = { id: current.id, name: current.name, sprite: current.sprite };
      continue;
    }
...
function selectRoute(dateKST, pack, tree, ctx, locked) {
  ...
  // (see README "로테이션 후보 요건" -- 가오몬 tops out at 궁극체, no 초궁극체
  // sprite exists to draw) and its last node marks `terminal: true` (D7 -
  // see docs/evolution-routes.md §8). Before D7, readTree required every
  // pack to declare all 7 global stages, so this defense against a short
  // tree could never actually fire in production - only in tests handing
  // selectRoute a short tree directly. D7 makes it live: readTree now
  // returns exactly these shorter, front-contiguous trees for real shipped
  // packs, so walk() has to cope with `tree[stage]` being undefined past
  // where the declared stages end - which it does via the terminal-fill
  // branch (see walk's doc comment), not by needing this list to bound
  // the loop anymore.
  ...
  return walk(dateKST, pack, tree, ctx, byId, locked, lockedThroughIdx);
}
...
function computeDailyTokens(now = new Date(), options = {}) {
  ...
  //
  // terminalFrom is a single top-level string on the result, not a field on
  // individual route nodes - readDailyState.swift casts route as
  // `[String: [String: String]]`; one non-string value anywhere in that
  // structure (a bool, or a string mixed in at the wrong nesting level)
  // collapses the whole cast to `[:]`, silently losing every stage's name
  // and sprite. Keeping route exactly `{id, name, sprite}` and carrying the
  // "why it stopped here" fact alongside it, not inside it, gets the same
  // information to the menubar without that risk (decision: D7 옵션1, see
  // docs/evolution-routes.md §8).
  let terminalFrom = null;
  if (tree) {
    for (const stageDef of TREE.stages) {
      const entry = route[stageDef.id];
      if (!entry) continue;
      const declared = tree[stageDef.id];
      const node = Array.isArray(declared) ? declared.find((n) => n.id === entry.id) : null;
