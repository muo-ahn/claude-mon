...
//
// Edges are checked in declaration order (childrenOf's ordering preserves
// this); the first edge whose `when` is met sets the day's winning
// condition, and every edge sharing that exact condition joins the pool.
// Zero met is no longer a data violation - the fallback (met.length > 0 ?
// met : links) guarantees at least one candidate as long as links itself is
// non-empty, which it is by construction (validateGraph's unreachable-node
// rule ensures that). Zero links means this is a terminal node, so return
// an empty array - the caller (selectRoute's forward phase) will detect
// this and repeat the same node for all remaining stages.
...
//
// CRITICAL: the hash key `${dateKST}|${pack}|${stage}` must NEVER change -
...
function selectRoute(dateKST, rookie, graph, ctx, locked) {
  ...
  // PREFIX: backward walk from rookie to digitama. Applies candidatesFor
  // logic (met -> fallback -> pick) to parent list for determinism. This
  // is pack isolation in Phase B - once digitama gets conditions (A-1),
  // delete this and start at digitama with forward walk.
  //
  ...
  for (let i = rookieStageIdx - 1; i >= 0; i--) {
    ...
    const met = parents.filter(p => conditionMet(p.when, ctx));
    const pool = met.length > 0 ? met : parents;
    const winner = JSON.stringify(pool[0].when);
    const tied = pool.filter(p => JSON.stringify(p.when) === winner);
    const parentNodes = tied.map(p => p.node);
    const chosen = parentNodes[hashString(`${dateKST}|${rookie}|${stage}`) % parentNodes.length];
  ...
  for (let i = rookieStageIdx; i < stageOrder.length; i++) {
    ...
    const node = candidates[hashString(`${dateKST}|${rookie}|${stage}`) % candidates.length];
    route[stage] = { id: node.id, name: node.name, sprite: node.sprite };
    currentNode = node;
  }
