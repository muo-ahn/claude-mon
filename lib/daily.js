...
// A pack is valid if it ships a digitama-0.png of its own, or the sprite
// root has sprites/shared/digitama-0.png -- a Digi-Egg looks the same
// whoever is inside it, so the menubar app (see claudemon-menubar.swift's
// sharedStages) falls back to the shared egg for any pack that doesn't draw
// its own. Mirrors that fallback here so a pack relying on the shared egg
// still counts as a rotation candidate. Sorted so the hash-based index in
// selectMon is stable regardless of directory listing order.
function listValidPacks(packsDir, sharedDir) {
  ...
  for (const entry of entries) {
    ...
    if (hasSharedDigitama || fs.existsSync(path.join(packsDir, entry.name, 'digitama-0.png'))) {
...
function selectMon(dateKST, packsDir, sharedDir) {
...
function computeDailyTokens(now = new Date(), options = {}) {
  ...
  const mon = (todaysDaily && todaysDaily.mon) || selectMon(dateKST, packsDir, sharedDir);
