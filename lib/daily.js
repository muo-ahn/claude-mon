...
//
// `"topStage": "<stageId>"` in pack.json is how a pack says "this is the
// end of the line, on purpose". Packs that say nothing are held to the
// tree's own top stage, so an unfinished line stays out of the rotation
// until it is finished rather than quietly settling for less.
...
// reaches the ceiling they claim. A pack that stops short of its own
// declared ceiling would spend the heaviest days of the month stuck below
// it, so it is held out of the rotation rather than shown with a stage it
// can't evolve into. Nothing else changes for it - it stays a valid pack,
// so an explicit selection or the guilmon fallback still renders it.
function listRotationPacks(packsDir, sharedDir) {
  return listValidPacks(packsDir, sharedDir).filter((pack) =>
    packDeclaresStage(packsDir, pack, packTopStage(packsDir, pack))
  );
}
...
function pruneTree(tree, hasDot, top) {
  ...
  const ceiling = stages[stages.length - 1];
  const byId = new Map();
  ...
  for (let i = stages.length - 1; i >= 0; i--) {
    ...
    for (const node of tree[stage] || []) {
      ...
      if (stage === ceiling || carriesOn) alive.add(node.id);
    }
  }
  ...
  const walk = (bumpStage) => {
    ...
    for (const stage of stages) {
      ...
      if (!current) {
        ...
        // pruneTree guarantees this can't happen; if it somehow does, give
        // up on the route rather than fabricate an edge to continue on.
