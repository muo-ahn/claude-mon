...
// Bumped whenever a field is added to a cache.files[] entry that old cache
// data can't supply (e.g. userTurns below). A version mismatch forces one
// full rescan rather than silently treating "field absent" as "field is
// zero forever" for files whose already-consumed bytes are gone from the
// scan window.
...
function loadCache(claudemonDir) {
  ...
  try {
    ...
    if (parsed.version !== CACHE_VERSION) {
      // Old cache shape - its files[] entries don't carry userTurns, and
      // there is no way to backfill that from an offset alone. Dropping
      // the whole cache (same as "file missing") forces one full rescan
      // from byte 0 of every transcript, after which the new version is
      // saved and every later run is incremental again.
      return { dateKST: null, files: {} };
    }
...
function scanFileIncrement(file, fromOffset, dayStartMs, dayEndMs, countUserTurns) {
...
function computeDailyTokens(now = new Date(), options = {}) {
  ...
  for (const file of files) {
    ...
    try {
      const { increment, newOffset, userTurnIncrement } =
        scanFileIncrement(file, fromOffset, dayStartMs, dayEndMs, countUserTurns);
      cache.files[file] = {
        offset: newOffset,
  ...
  // copy, since both are appended from the same stream of turns. userTurns
  // rides along with whichever copy wins on contribution, rather than
  // being maxed independently, since both fields grow off the same stream.
  ...
  for (const [file, entry] of Object.entries(cache.files)) {
    ...
    if (!contribution && !userTurns) continue;
    ...
    if (!previous || contribution > previous.contribution) {
      byTranscript.set(identity.key, { contribution, userTurns, sessionId: identity.sessionId });
    }
  }
  ...
  const sessionTokens = {};
  for (const { contribution, userTurns, sessionId } of byTranscript.values()) {
    outputTokens += contribution;
    dailyUserTurns += userTurns;
