...
//   2. Spine return. A branch that dead-ends (DWDS has no Mega for
//      지오그레이몬, and none of the armour forms carry on) hands the next
//      stage back to the spine. The route therefore always reaches the top
//      stage, so hitting 2M is never wasted on a line that can't evolve.
//
...
// the stages above it against today's traits. That is what lets a trait
// earned mid-day (e.g. swarm, which cannot be true in the first seconds of
// the day) still steer a branch the mon hasn't reached yet, while the form
// on screen right now never flickers. If the locked route doesn't line up
// with the current tree (the tree changed since it was pinned - see the
// 라이즈그레이몬/샤인그레이몬 case), that stage and everything after it
// degrades to a normal draw instead of throwing.
// Draws one node from `candidates` for `stage`, biased toward whichever of
// them carry a trait the day actually earned (see traits, above `selectRoute`
// doc comment). No match leaves every candidate in the draw.
function pick(dateKST, pack, stage, candidates, bump, traits) {
...
// Walks `stages` top to bottom, applying `locked` and nudging `bumpStage`
// (see selectRoute's doc comment for what each does). Returns one node per
// stage plus which stages actually had more than one candidate, so
// selectRoute can find "the last stage that had alternatives" to nudge.
function walk(dateKST, pack, tree, traits, byId, stages, locked, lockedThroughIdx, bumpStage) {
  const route = {};
  const branching = [];
  let current = null;
  ...
  for (const stage of stages) {
    if (lockedActive && TREE.stages.findIndex((s) => s.id === stage) <= lockedThroughIdx) {
      ...
      if (node) {
        ...
        current = node;
        continue; // pinned stages never enter branching - they can't be re-nudged
      }
    ...
    if (!current) {
      ...
      if (candidates.length === 0) candidates = [tree[stage][0]]; // spine return
    }
    const { node, choices } = pick(dateKST, pack, stage, candidates, bumpStage === stage ? 1 : 0, traits);
...
function selectRoute(dateKST, pack, tree, traits, avoidRoute, locked) {
  ...
  const first = walk(dateKST, pack, tree, traits, byId, stages, locked, lockedThroughIdx, null);
  ...
  return walk(dateKST, pack, tree, traits, byId, stages, locked, lockedThroughIdx,
              first.branching[first.branching.length - 1]).route;
}
...
function computeDailyTokens(now = new Date(), options = {}) {
  ...
  if (tree) {
    ...
    // had and only re-draws the stages still ahead against today's traits
    // - see selectRoute's `locked` param. That is what lets a trait like
    // swarm, which cannot be true in the first seconds of the day, still
    // steer a branch the mon hasn't reached yet without the on-screen form
    // ever flickering.
    const pinned = previous && previous.dateKST === dateKST && previous.mon === mon && previous.route
      ? previous.route
      : null;
    prevRoute = pinned
      ? (previous.prevRoute || null)
      : ((previous && previous.mon === mon && previous.route) || null);
