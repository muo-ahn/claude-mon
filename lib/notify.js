...
//     passthrough envelope. Requires `allow-passthrough on` in tmux;
//     without it tmux swallows the sequence and nothing happens.
...
function tmuxContext() {
  ...
  try {
    const out = execFileSync(
      'tmux',
      ['display-message', '-p', '-t', pane, '#{client_termname}\n#{pane_tty}\n#{pane_title}'],
      { encoding: 'utf8', timeout: 2000 }
    );
    const [termname = '', tty = '', title = ''] = out.split('\n');
    return { termname: termname.trim(), tty: tty.trim(), title: title.trim() };
  } catch (e) {
    return null;
  }
}
...
function notify({ title, body, urgency }) {
  ...
  const target = resolveTarget(tmux);
