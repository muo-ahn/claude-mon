...
// Desktop banners, in three tiers: Claudemon.app first, kitty's OSC 99
// protocol second, an audible fallback last.
//
// Claudemon.app is preferred because macOS attributes a notification to the
// app bundle that posted it - going through kitty meant every banner was
// labeled "kitty" and carried kitty's icon. The app also removes the
// terminal from the equation entirely (Ghostty, Terminal.app and a bare tty
// all work), and it can report whether notification permission is actually
// granted, which the escape-code path cannot.
//
...
// Claudemon.app's queue. Must match notifyQueueDir in claudemon-menubar.swift.
const NOTIFY_QUEUE_DIR = path.join(STATE_DIR, 'notify-queue');
const HEARTBEAT_FILE = path.join(STATE_DIR, 'menubar-alive.json');
// The app rewrites the heartbeat every 2s, so anything older than this means
// it is gone. Kept well above the tick so a slow tick doesn't drop the path.
...
// Whether Claudemon.app can take a notification right now. The app writes a
// heartbeat every 2s carrying both its liveness and its authorization state -
// a running app with permission denied would swallow the notification, so
// both conditions are read together and a stale or unauthorized heartbeat
// sends the caller down to the escape-code path.
function claudemonReady(now = Date.now()) {
  try {
    const heartbeat = JSON.parse(fs.readFileSync(HEARTBEAT_FILE, 'utf8'));
    if (!heartbeat.authorized) return false;
    const at = Number(heartbeat.at) * 1000;
...
// Distinguishes requests queued within the same millisecond. Epoch ms alone
// is not enough: two notifications can be queued back to back (a turn ending
// and a sub-agent finishing), and with an identical timestamp and pid the
// sort fell through to the random suffix, so the app posted them in arbitrary
// order. Zero-padded so a lexical sort matches numeric order.
let queueSequence = 0;

// Drops one request into the queue. The filename leads with epoch ms plus the
// sequence above so the app gets arrival order from a plain sort. Written as
// .tmp then renamed because the app polls every 0.5s and would otherwise be
// able to read a half-written file - it only picks up names ending in .json,
// and rename within a directory is atomic.
function deliverViaApp(payload) {
  try {
    fs.mkdirSync(NOTIFY_QUEUE_DIR, { recursive: true });
    const seq = String(queueSequence++).padStart(4, '0');
    const stamp = `${Date.now()}-${seq}-${process.pid}-${Math.random().toString(36).slice(2, 8)}`;
    const finalPath = path.join(NOTIFY_QUEUE_DIR, `${stamp}.json`);
    fs.writeFileSync(`${finalPath}.tmp`, JSON.stringify(payload));
...
// Returns 'app' when Claudemon.app took it, 'banner' when a banner reached a
// kitty terminal, 'sound' when it fell back to audio, or false when nothing
// was delivered.
//
...
function notify({ title, body, subtitle, urgency, threadId }) {
  if (!notificationsEnabled()) return false;

  if (claudemonReady() && deliverViaApp({ title, body, subtitle, urgency, threadId })) {
    return 'app';
  }
...
// threadId groups a project's notifications together in Notification Center.
// Only the app path uses it; the escape-code path ignores the extra field.
function compose(subject, projectName) {
  return {
    title: sessionTitle(tmuxContext()) || projectName || 'claudemon',
    body: subject,
    threadId: projectName || undefined
  };
}
